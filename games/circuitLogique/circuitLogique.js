...
// --- SONS (mettre les fichiers, par ex., dans games/circuitLogique/sfx/) ---
let sfxFaucet, sfxSuccess, sfxError;
try {
...
// ---------- Logique ----------
function startGame() {
  ScoreService.init("circuitLogique");
  ScoreService.resetScore().catch(err =>
    console.warn("resetScore circuitLogique:", err)
  );
  ...
  if (valid) {
    ...
    if (!puzzleAlreadyCounted) {
      ...
      try {
        ...
        console.warn("addPoints circuitLogique:", e);
      }
    }
...
async function endGame() {
  ...
  try {
    await ScoreService.saveScore("circuitLogique", solvedCount);
  } catch (e) {
    console.warn("saveScore circuitLogique:", e);
  }
  ...
  try {
    ...
    console.warn("getScore circuitLogique:", e);
